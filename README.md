...
v0.3.0 · Vue 3 + TypeScript + Vite · Tauri 2（桌面端）· Capacitor（安卓端）
...
进入工作区 → 批量导入 → 水印工作室 → 快速辑录 → 打包导出。所有图像处理都在本机
完成，**没有任何上传**，也没有账号与云端的负担。

## 功能

### 1. 工作区
- 启动即见工作区入口：**新建工作区**（命名 + 选位置）或从**最近使用**进入；
  进入工作区后才能导入照片、打开操作面板
- 工作区本质是一个文件夹（`workspace.json` + `images/`），两种照片组织方式：
  - **复制**：照片拷贝进工作区，源文件增删不影响
...
- **文本图层**：字体（含**扫描系统字体 / 手动输入**的自定义字体）、字重、颜色、字距、
  行高、对齐、斜体、描边、阴影、**背景填充**
...
双击滑杆名称复位单项。默认应用到全部照片；对某张照片开启**单独调节**后，
它使用独立参数，不再应用全局调节。
...
├─ stores/      # Pinia：workspace / images / watermark / presets / adjust
│               # templates / export / settings
├─ components/  # 标题栏 · 工作区启动器 · 预览画布（双层）· 水印工作室 · 图层编辑器 · ui/ 原子组件
...
- 文件夹工作区需要桌面端；安卓端为会话级临时工作区
- 大体积 ZIP（数百张以上）在浏览器端会占用较多内存；桌面端后续改为流式写盘
- 扫描系统字体依赖 Chromium 内核（WebView2 等），Safari/WebKit 环境可手动输入字体名
